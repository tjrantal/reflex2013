...
constants.htmlFolder =  [constants.baseFolder separator 'analysis' separator 'htmlRun'];
...
for p = 1:length(constants.subjectFolders)
    ...
    for f = 1:length(fileList); %Go through files in a directory % 'analysis' separator 
		...
		fprintf(htmlFile,'<img src=\"%s\" width=\"1500px\"/></br>\n',['..' htmlSeparator 'groundContactVisualization' htmlSeparator constants.subjectFolders(p).dir.name htmlSeparator fileList(f).name]);
