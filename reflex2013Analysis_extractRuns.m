...
constants.stretchFolder =[constants.baseFolder separator 'analysis' separator 'groundContacts'];
...
for p = 1:length(constants.subjectFolders) %
    ...
    for f = 1:length(fileList); %Go through files in a directory
		...
		if strfind(lower(fileList(f).name),lower(constants.visualizationTitles{10})) > 0
			%keyboard
			runData = extractRun(data,synchronization,constants,2,fileList(f).name,f);
		end
